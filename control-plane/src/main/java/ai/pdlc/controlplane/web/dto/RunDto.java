 ...
 * {@code outputSchema}. Token counts are {@code null} when the provider did not report them. An a2a
 * run (slice 2.5) has no model; {@code remote} is the remote task it follows (only on single-run reads).
 */
public record RunDto(String id, String workspaceId, String agentId, int agentVersion, String contentHash, String model,
                     ...
                     OffsetDateTime createdAt, OffsetDateTime startedAt, OffsetDateTime finishedAt, Remote remote) {

    /**
     * {@code state} is the remote agent's own task state; {@code question} its status message (what
     * it asked when waiting for input or authorization); {@code cancel} what it did with a cancel.
     */
    public record Remote(String protocolVersion, String taskId, String contextId, String state, String question,
                         String cancel) {
    }
}
